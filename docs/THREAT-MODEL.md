...
|---|---|---|---|---|
| Client compromise (subscriber device) | M | M | Secrets never ship to clients; access code is a bearer token scoped to one hash; revocation kills it centrally | Local SQLite (research/memory/paper state on device) is readable by whoever controls the device |
...
| Database compromise (Neon) | L | H | Codes stored hashed; orders store code plaintext ONLY tied to one order for retrieval; no card/PII data stored | Paper portfolios + orders visible if DB leaks; no real-money impact by design |
...
| Cross-user isolation failure | L | M | Paper store scoped by code hash at query level (tested); no cross-user read path exists | True multi-tenant accounts NOT implemented (single-operator-per-code model) |
