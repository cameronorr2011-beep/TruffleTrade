// TruffleTrade v7 — "The AI stock trader that shows its receipts."
// Same 3D canvas film as v6, rescored and rewritten for a mass audience:
//   • music: NCS "Symbolism" (Electro-Light) — INSTRUMENTAL, huge, powerful
//     (v6's vocal track fought the voiceover). Cut 50.85s→80.85s: quiet
//     tension under the pitch, first-drop tail under the red team, a hard
//     pre-drop cut, then the DROP lands at 17.0s exactly when the memory
//     wall lights up — the product IS the drop.
//   • audio: same no-pump chain as v6 (shared -14 LUFS normalize, 60%
//     sidechain floor, -1.5 dBTP ceiling).
//   • copy: plain English, explicitly "AI STOCK TRADER"; jargon is on
//     small chips only. End card: "AN AI STOCK TRADER THAT SHOWS ITS RECEIPTS."
...
// --- music source: NCS "Symbolism" — Electro-Light (instrumental) ---
// CUT_IN = 50.85 puts the track's BIG DROP (measured at song 67.85) at video
// t=17.0 — the exact moment the memory wall lights up. Beats: 0.703s period
// (fit on drop-section kicks), phase 0.128 so a beat lands on the drop itself.
const MUSIC_FILE = path.join(OUT, "ncs-symbolism.mp3");
const CUT_IN = 50.85;
const MUSIC_GAIN = Number(process.env.TT_MUSIC_GAIN ?? 0.55); // v7: higher bed — this cut must feel POWERFUL
const DUCK_FLOOR = Number(process.env.TT_DUCK_FLOOR ?? 0.60); // sidechain never below 60% (no pumping)
const PREVIEW = process.env.TT_PREVIEW === "1";               // render every 15th frame to PNGs only

// --- beat grid (drop-section kick fit: 703ms per beat) ---
const BEAT = 0.703;
const PHASE = 0.128;        // beats at video t ≡ 0.128 (mod 0.703) — one ON the drop at 17.0
const dropAt = 17.0;        // the drop: song 67.85
const beatPulse = (t, shift = 0) => Math.max(0, 1 - ((((t - shift) - PHASE) % BEAT + BEAT) % BEAT) / BEAT) ** 3;
...
function chCanyon(ctx, t) {
  ...
  // title — v7: say what the product is, in plain words, in the first second
  const a1 = segOut(t, 0.3, 1.1);
  brand(ctx, a1);
  txt(ctx, "AI READS THE", W / 2, H * 0.30 + (1 - a1) * 30, 92, { alpha: a1, track: 4 });
  txt(ctx, "STOCK MARKET.", W / 2, H * 0.30 + 110 + (1 - a1) * 30, 92, { alpha: a1, color: GOLD, track: 2, glow: hexA(GOLD, 0.5) });
  chip(ctx, "EVERY CHART · EVERY DAY", H * 0.42, BONE_SOFT, segOut(t, 1.1, 1.6), 26);
  void k;
}
...
function chMonolith(ctx, t) {
  ...
  // v6: continuous slow push-in + sway across the WHOLE chapter (v5 froze after 3.5s)
  const cam = { x: 70 * Math.sin(l * 0.3 + 0.6), y: 40 - l * 2.2, z: -520 + segOut(l, 0, 6.0) * 170, yaw: 0.13 * Math.sin(l * 0.3 + 1.2), pitch: -0.03, f: 900 };
  ...
  // the lone grey orb — v6: pulse locked to the song's kick grid instead of a free-running sine
  const pulse = 1 + 0.055 * beatPulse(t);
  ...
  txt(ctx, "MOST APPS GIVE YOU", W / 2, H * 0.17, 70, { alpha: segOut(l, 0.2, 0.9), color: "#c9c2b8", track: 4 });
  txt(ctx, "ONE ANSWER. NO RECEIPTS.", W / 2, H * 0.17 + 84, 70, { alpha: segOut(l, 0.5, 1.2), color: "#c9c2b8", track: 2 });
}

function councilScene(ctx, t, opts) {
  const { l, dim = 0, red = 0, refuse = 0 } = opts;
  bg(ctx, refuse ? RED : GOLD, refuse ? 0.10 : 0.16, W * 0.5, H * 0.35);
  // v6: slower settle (1.5s) so chapter cuts don't feel like camera jumps,
  // plus a continuous gentle sway so the frame never goes fully static
  const cam = { x: 14 * Math.sin(l * 0.35), y: 60 + 6 * Math.sin(l * 0.5), z: -560 + segOut(l, 0, 1.5) * 90, yaw: 0.02 * Math.sin(l * 0.35), pitch: -0.02, f: 900 };
  floorGrid(ctx, cam, -200, refuse ? RED : GOLD, 0.09, -300, 900, 80, 800);
  // holographic chart at center — v6: holo-glow breathes on the kick grid
  const chartCorners = rect3(0, 40, 220, 560, 348);
  plane(ctx, CHART_TEX, chartCorners, cam, { fill: "#0e0c0a", shadow: true, glow: (0.55 + 0.18 * beatPulse(t)) * (1 - 0.72 * refuse), glowColor: refuse ? "#6a3a30" : GOLD, dim: refuse ? 0.6 : 0 });
  // nine nodes orbit on a tilted ring — v6: calmer drift, beat-staggered bob + glow
  const ring = [];
  for (let i = 0; i < 9; i++) {
    const ang = (i / 9) * Math.PI * 2 + l * 0.42;
    const R = 330;
    const x = Math.cos(ang) * R, z = 220 + Math.sin(ang) * 120;
    const y = 40 + Math.sin(ang) * 150 + beatPulse(t, i * 0.11) * 9;
  ...
  for (const n of ring) {
    ...
    orb(ctx, n.x, n.y, n.z, 26 * appear * (1 + 0.07 * beatPulse(t, n.i * 0.11)), color, cam, appear * (1 - dim), label);
...
function chCouncil(ctx, t) {
  ...
  txt(ctx, "NINE AI ANALYSTS DEBATE.", W / 2, H * 0.16, 66, { alpha: segOut(l, 0.4, 1.1), track: 3 });
...
function chRedTeam(ctx, t) {
  ...
  // v6: red pulse locked to the beat grid (was a free-running 9 rad/s sine)
  ctx.save(); ctx.globalAlpha = 0.08 + 0.07 * beatPulse(t, 0.165); ctx.fillStyle = RED; ctx.fillRect(0, 0, W, H); ctx.restore();
...
function chRefuse(ctx, t) {
  ...
  txt(ctx, "WEAK EVIDENCE?", W / 2, H * 0.16, 66, { alpha: segOut(l, 0.2, 0.9), track: 3 });
  txt(ctx, "IT SAYS NO.", W / 2, H * 0.16 + 82, 74, { alpha: segOut(l, 0.6, 1.3), color: RED, track: 3, glow: hexA(RED, 0.5) });
}
function chMemory(ctx, t) {
  const l = t - CH.memory;
  bg(ctx, GREEN, 0.12, W * 0.2, H * 0.3);
  // v6: eased travel settling at 3.4s, then a continuous slow push-in on the wall
  const cam = { x: -260 + segOut(l, 0, 3.4) * 520, y: 40, z: -640 + seg(l, 0, 8.4) * 80, yaw: -0.18 + segOut(l, 0, 3.4) * 0.36, pitch: -0.01, f: 950 };
  ...
  ctx.restore();
  txt(ctx, "IT SHOWS ITS RECEIPTS.", W / 2, H * 0.16, 66, { alpha: segOut(l, 0.2, 0.9), track: 4 });
  txt(ctx, "EVEN THE MISSES.", W / 2, H * 0.16 + 86, 70, { alpha: segOut(l, 0.6, 1.3), color: GREEN, track: 2, glow: hexA(GREEN, 0.5) });
...
function chRoom(ctx, t) {
  ...
  // v6: monitor glow kicks on the beat grid (was a free-running sine)
  drawRoom(ctx, cam, t, 0.5 + 0.22 * beatPulse(t));
  txt(ctx, "AN AI STOCK TRADER.", W / 2, H * 0.16, 64, { alpha: segOut(l, 0.3, 1.0), track: 5, glow: hexA(GOLD, 0.35) });
  txt(ctx, "ON YOUR DESKTOP.", W / 2, H * 0.16 + 78, 64, { alpha: segOut(l, 0.7, 1.3), color: GOLD, track: 5, glow: hexA(GOLD, 0.3) });
}
function endCard(ctx, t) {
  const l = t - CH.end;
  // v6: the glow drifts and breathes so the settled end card never goes static
  bg(ctx, GOLD, 0.18 + 0.05 * beatPulse(t) + 0.02 * Math.sin(t * 0.7), W / 2 + 46 * Math.sin(t * 0.24), H * 0.34 + 34 * Math.cos(t * 0.19));
  const fade = (a, b) => segOut(l, a, b);
  // v6: logo scale bounces on the beat grid (was a free-running sine)
  const s = 260 + 14 * beatPulse(t);
  ...
  txt(ctx, "AN AI STOCK TRADER", W / 2, H * 0.475, 68, { alpha: fade(0.35, 0.8), track: 4 });
  txt(ctx, "THAT SHOWS ITS RECEIPTS.", W / 2, H * 0.475 + 92, 68, { alpha: fade(0.6, 1.05), color: GOLD, track: 3, glow: hexA(GOLD, 0.5) });
  chip(ctx, "FREE WINDOWS APP · THEN 1,000 SATS/MO · NO KYC", H * 0.622, GREEN, fade(1.0, 1.4), 24);
  ...
  txt(ctx, 'MUSIC: "SYMBOLISM" — ELECTRO-LIGHT · NCS', W / 2, H * 0.77, 19, { alpha: fade(1.8, 2.2), color: FAINT, weight: 600, font: MONO });
}
...
// ---------- audio ----------
/** Decode any audio file to 48k stereo s16le and loudness-normalize it.
 *  Normalizing each source to the same LUFS target BEFORE mixing is what kills
 *  the v4/v5 pumping: the bed no longer fights a quieter/louder master. */
function loadNormalized(file, targetI = -14) {
  const raw = path.join(OUT, `v6-norm-${path.basename(file)}.pcm`);
  execFileSync(ffmpegPath, [
    "-y", "-i", file, "-ac", "2", "-ar", "48000",
    "-af", `loudnorm=I=${targetI}:TP=-1.5:LRA=11`,
    "-f", "s16le", raw,
  ], { stdio: ["ignore", "ignore", "ignore"] });
  const pcm = fs.readFileSync(raw);
  fs.rmSync(raw, { force: true });
  return pcm; // interleaved s16le stereo
}
...
function buildAudio() {
  ...
  // --- music bed: loudness-normalized to the same target as the VO ---
  const music = loadNormalized(MUSIC_FILE, -14);
  const mf = music.length >> 2;
  const off = Math.round(CUT_IN * SR);
  // arrangement envelope: gentle build through the VO lines, full bed at the
  // drop, small swell across the end card
  // v7 arrangement: gentle open, STRONG from 7.0 (first-drop tail = red team),
  // hard cut to near-silence for the 2s pre-drop tension, full power at the drop
  const mArr = (tv) => {
    if (tv < 1.4) return 0.55 + 0.25 * (tv / 1.4);   // soft open (0.55→0.80)
    if (tv < 7.0) return 0.80;                        // quiet tension under l2-l4
    if (tv < dropAt - 2.0) return 1.05;               // first-drop tail (red team)
    if (tv < dropAt) return 0.30;                     // pre-drop choke (15.0→17.0)
    if (tv > CH.end + 1.0) return 1.12;               // end-card swell
    return 1.0;                                       // full power after the drop
  };
  for (let i = 0; i < Math.min(mf - off, N); i++) {
    const g = MUSIC_GAIN * mArr(i / SR);
    L[i] = music.readInt16LE((off + i) * 4) / 32768 * g;
    R[i] = music.readInt16LE((off + i) * 4 + 2) / 32768 * g;
  }

  // --- gentle sidechain under VO (floor 60%, 0.35s ramps): shape, not pump ---
  const duck = new Float32Array(N).fill(1);
  const F = 0.35 * SR;
  for (const line of VO) {
    const s = at[line.id] * SR, e = (at[line.id] + line.dur) * SR;
  ...
  // v7: no breakdown lift — Symbolism's energy carries the whole film; the
  // pre-drop choke (15→17s) stays quiet ON PURPOSE for maximum impact

  // --- voiceover: same loudness target as the bed, then +7.5 dB of presence ---
  const VO_GAIN = 2.37; // ≈ +7.5 dB over the shared -14 LUFS bed
  for (const line of VO) {
    const v = loadNormalized(line.file, -14);
    const vf = v.length >> 2;
    const start = Math.floor(at[line.id] * SR);
    for (let i = 0; i < vf; i++) {
      const idx = start + i; if (idx >= N) break;
      const tin = i / SR, tout = vf / SR - tin;
      const env = Math.min(1, tin / 0.02, tout / 0.05) * VO_GAIN;
      L[idx] += v.readInt16LE(i * 4) / 32768 * env;
      R[idx] += v.readInt16LE(i * 4 + 2) / 32768 * env;
    }
  }

  // --- master: soft clip, fades, then a fixed true-peak ceiling (-1.5 dBTP) ---
  const CEIL = Math.pow(10, -1.5 / 20);
  let peak = 1e-9;
  for (let i = 0; i < N; i++) {
    const t = i / SR;
    let l = Math.tanh(L[i] * 0.95), r = Math.tanh(R[i] * 0.95);
  ...
  const norm = Math.min(CEIL / peak, 2.0); // trim to the ceiling; never boost past 2×
  ...
  const p = path.join(OUT, "v7-audio.wav");
...
if (PREVIEW) {
  fs.mkdirSync(path.join(OUT, "v7-preview"), { recursive: true });
  for (let fr = 0; fr < FRAMES; fr += 15) {
    seed = 424242 + fr * 7919;
    renderFrame(sc, fr / FPS);
    ctx.drawImage(scC, 0, 0); ctx.drawImage(VIG, 0, 0);
    fs.writeFileSync(path.join(OUT, "v7-preview", `f${String(fr).padStart(4, "0")}.jpg`), main.toBuffer("image/jpeg", 72));
  }
  console.log("preview frames written to dist-video/v7-preview");
  process.exit(0);
}
...
const outMp4 = path.join(OUT, "truffletrade-v7.mp4");
...
for (const s of [1.5, 4, 9, 12.5, 16.5, 20, 24, DUR - 2.5]) {
  execFileSync(ffmpegPath, ["-y", "-ss", String(s), "-i", outMp4, "-frames:v", "1", "-vf", "scale=306:544", "-update", "1", path.join(OUT, `v7ps-${String(s).replace(".", "_")}.png`)], { stdio: "ignore" });
}
