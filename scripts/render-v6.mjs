// TruffleTrade v6 — "See every side."
// A vertical (1080×1920) 30s YouTube-Short / homepage brand film rendered
// frame-by-frame in node-canvas with a tiny perspective camera. Seven chapters,
// each motivated by one VO line (ElevenLabs clips + timings from render-vo-v4.mjs):
...
// v6 changes vs v5:
//   • music: NCS "Seasons" (Rival x Cadmium feat. Harley Bird) — lyrical female
//     vocals. Cut 101.384s→131.384s, beat-grid aligned so the arrangement drives
//     the edit: groove tail under line 1, quiet breakdown under lines 2-6,
//     pre-drop riser builds under the memory wall, and the DROP lands exactly on
//     the room/product reveal (t≈21.6), carrying the end card out.
//   • audio: single loudness-normalized master chain (music & VO normalized
//     to the same -14 LUFS target BEFORE mixing) → no pumping; gentle sidechain
//     that never dips below 60% replaces the 12% duck; breakdown section is
//     lifted ~+5 dB so the quiet passage still breathes; final peak -1.5 dBTP.
//   • sync: beats are 0.6616s (kick-locked fit), video t=0 is ON a beat, so every
//     visual pulse lands on the real kick grid; drop impact measured at t≈21.62.
//   • motion: eased 1.5s transitions with 0.15s settle, no camera jumps
...
// --- music source: NCS "Seasons" — Rival x Cadmium feat. Harley Bird ---
// CUT_IN is the song timestamp whose instant becomes video t=0, chosen ON the
// fitted beat grid (kicks at song 123.217 + k×0.6616) and 33 beats before the
// drop, so video t=0 is beat-aligned and the breakdown covers the VO lines.
const MUSIC_FILE = path.join(OUT, "ncs-seasons.mp3");
const CUT_IN = 101.384;
const MUSIC_GAIN = Number(process.env.TT_MUSIC_GAIN ?? 0.42); // base bed level under VO
...
// --- beat grid (kick-locked fit: 661.6ms per beat, video t=0 lands on a kick) ---
const BEAT = 0.6616;
const PHASE = 0;            // beats occur at video t = 0, 0.6616, 1.3232, …
const dropAt = 21.62;       // measured drop impact (song 123.0) in video time
...
function endCard(ctx, t) {
  ...
  txt(ctx, 'MUSIC: "SEASONS" — RIVAL X CADMIUM FT. HARLEY BIRD · NCS', W / 2, H * 0.77, 19, { alpha: fade(1.8, 2.2), color: FAINT, weight: 600, font: MONO });
}
...
function buildAudio() {
  ...
  const mArr = (tv) => {
    if (tv < dropAt - 3.4) return 0.62 + 0.38 * seg(tv / Math.max(0.01, dropAt - 3.4), 0, 1);
    if (tv < dropAt) return 1.0;
    if (tv > CH.end + 1.0) return 1.1; // end-card swell
    return 1.0;
  };
  ...
  // --- breakdown lift: the song's quiet passage (video ~1.3→drop-2.0s) gets a
  // gentle make-up so the mix never collapses into near-silence under the VO ---
  for (let i = 0; i < N; i++) {
    const t = i / SR;
    if (t > 1.3 && t < dropAt - 2.0) { L[i] *= 1.6; R[i] *= 1.6; }
  }
  ...
  const p = path.join(OUT, "v6-audio.wav");
...
if (PREVIEW) {
  fs.mkdirSync(path.join(OUT, "v6-preview"), { recursive: true });
  for (let fr = 0; fr < FRAMES; fr += 15) {
    seed = 424242 + fr * 7919;
    renderFrame(sc, fr / FPS);
    ctx.drawImage(scC, 0, 0); ctx.drawImage(VIG, 0, 0);
    fs.writeFileSync(path.join(OUT, "v6-preview", `f${String(fr).padStart(4, "0")}.jpg`), main.toBuffer("image/jpeg", 72));
  }
  console.log("preview frames written to dist-video/v6-preview");
  process.exit(0);
}
...
const outMp4 = path.join(OUT, "truffletrade-v6.mp4");
...
for (const s of [1.5, 4, 9, 12.5, 16.5, 20, 24, DUR - 2.5]) {
  execFileSync(ffmpegPath, ["-y", "-ss", String(s), "-i", outMp4, "-frames:v", "1", "-vf", "scale=306:544", "-update", "1", path.join(OUT, `v6ps-${String(s).replace(".", "_")}.png`)], { stdio: "ignore" });
}
