  ...
  return (
    <section className="grid gap-5 lg:grid-cols-2">
      <div className="card p-6">
        <h2 className="font-mono text-[0.62rem] uppercase tracking-[0.28em] text-bone-soft">DCF — model scenario</h2>
        {dcf ? (
          <>
            <p className="mt-3">
              <span className="font-display text-[1.8rem] font-semibold text-ink">{usd(dcf.fairValue)}</span>
              <span className="ml-2 font-mono text-[0.62rem] uppercase tracking-[0.18em] text-faint">
            ...
            <table className="mt-4 w-full text-left font-mono text-[0.68rem]">
              <tbody>
                {[
                  ...
                  <tr key={k} className="border-b border-soil-600 last:border-0">
                    <td className="py-1.5 text-faint">{k}</td>
                    <td className="py-1.5 text-right text-bone-soft">{v}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <p className="mt-4 font-mono text-[0.58rem] uppercase tracking-[0.2em] text-faint">
              Sensitivity — discount rate × terminal growth
            </p>
            <div className="mt-2 grid grid-cols-3 gap-1.5">
              {dcf.sensitivity.map((s, i) => (
                <div key={i} className="rounded-md border border-soil-600 bg-soil-900/50 px-2 py-1.5 text-center">
                  <p className="font-mono text-[0.54rem] text-faint">
                    r{(s.discountRate * 100).toFixed(0)}% g{(s.terminalGrowth * 100).toFixed(1)}%
                  </p>
                  <p className="font-mono text-[0.72rem] text-bone-soft">{usd(s.fairValue)}</p>
                </div>
              ))}
            </div>
          ...
          <p className="mt-4 rounded-lg border border-soil-600 bg-soil-950 p-4 font-mono text-[0.72rem] leading-relaxed text-bone-soft">
      ...
      <div className="card p-6">
        ...
        {reverseDcf ? (
          <p className="mt-3 text-[0.86rem] leading-relaxed text-bone-soft">
            The current price of <strong className="text-ink">{usd(reverseDcf.price)}</strong> implies{" "}
        ...
        {comps.length > 0 ? (
          <table className="mt-5 w-full text-left">
            ...
            <tbody>
              {comps.map((c) => (
                <tr key={c.metric} className="border-b border-soil-600 last:border-0">
                  <td className="py-2 text-[0.78rem] text-bone-soft">{c.metric}</td>
                  <td className="py-2 text-right font-mono text-[0.72rem] text-ink">
                    {c.value == null ? <span className="text-faint">unavailable</span> : c.value.toFixed(1)}
                  </td>
                  <td className="py-2 text-right font-mono text-[0.72rem] text-bone-soft">
                    {c.peerMedian == null ? <span className="text-faint">unavailable</span> : c.peerMedian.toFixed(1)}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <p className="mt-4 font-mono text-[0.7rem] text-faint">Peer comps: DATA UNAVAILABLE.</p>
        )}
      </div>
    </section>
  );
