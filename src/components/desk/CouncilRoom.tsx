  ...
  if (!council) {
    return (
      <div className="card p-6">
        <h2 className="font-mono text-[0.62rem] uppercase tracking-[0.28em] text-bone/50">Council room</h2>
        <p className="mt-4 font-mono text-[0.72rem] text-bone/40">
  ...
  return (
    <div className="card p-6">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h2 className="font-mono text-[0.62rem] uppercase tracking-[0.28em] text-bone/50">Council room</h2>
        <span className="font-mono text-[0.62rem] text-bone/35">
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {council.votes.map((v) => (
          <article key={v.agent} className="rounded-xl border border-pit-300/12 bg-pit-950/60 p-4">
            ...
            <p className="mt-2 text-[0.78rem] leading-relaxed text-bone/50">{v.rationale}</p>
          </article>
        ))}

        <article className={`rounded-xl border p-4 ${council.redTeam.approved ? "border-jade/40 bg-jade/5" : "border-blood/40 bg-blood/5"}`}>
          <div className="flex items-center justify-between gap-2">
            <span className="font-mono text-[0.72rem] font-semibold tracking-[0.12em] text-bone">RED TEAM</span>
          ...
          {council.redTeam.objections.length > 0 ? (
            <ul className="mt-2 list-disc space-y-1 pl-4 text-[0.78rem] leading-relaxed text-bone/50">
              {council.redTeam.objections.slice(0, 3).map((o, i) => (
                <li key={i}>{o}</li>
              ))}
            </ul>
          ) : (
            <p className="mt-2 text-[0.78rem] leading-relaxed text-bone/50">{council.redTeam.notes || "no objections raised"}</p>
          )}
        </article>
      </div>
    </div>
