  ...
  return (
    <div className="card p-6">
      <div className="flex items-center justify-between gap-2">
        <p className="font-mono text-[0.6rem] uppercase tracking-[0.24em] text-bone-soft">Degen wallet · Base</p>
        <span className={`rounded-full px-2.5 py-1 font-mono text-[0.56rem] uppercase tracking-[0.18em] ${status.enabled ? "bg-jade/15 text-jade" : "bg-soil-800/60 text-bone-soft"}`}>
          {status.enabled ? "live on-chain" : status.configured ? "misconfigured" : "locked"}
        </span>
      </div>
      {status.enabled && status.address ? (
        <>
          <p className="mt-3 font-mono text-[0.7rem] text-bone-soft">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 font-mono text-[0.7rem]">
            <div className="rounded-lg bg-soil-950/70 p-2.5">
              <span className="text-bone-soft">USDC</span>
              <p className="text-ink">${(status.usdc ?? 0).toFixed(2)}</p>
            </div>
            <div className="rounded-lg bg-soil-950/70 p-2.5">
              <span className="text-bone-soft">cbBTC</span>
              <p className="text-ink">{(status.cbbtc ?? 0).toFixed(6)}</p>
            </div>
            <div className="rounded-lg bg-soil-950/70 p-2.5">
              <span className="text-bone-soft">gas ETH</span>
              <p className={status.gasEth !== undefined && status.gasEth > 0.0005 ? "text-jade" : "text-blood"}>
                {(status.gasEth ?? 0).toFixed(5)}
              </p>
            </div>
            <div className="rounded-lg bg-soil-950/70 p-2.5">
              <span className="text-bone-soft">cap/trade</span>
              <p className="text-ink">${status.maxUsdPerTrade}</p>
            </div>
          </div>
        </>
      ) : (
      ...
      <p className="mt-3 font-mono text-[0.6rem] text-bone-soft">
