...
export async function POST(req: Request) {
  ...
  if (action === "issue") {
    ...
    auditEvent("license_issued", hash, { orderId: "comped", days: 30 });
  ...
  if (!row) {
    auditEvent("admin_action_failed", "admin", { reason: "unknown_code" });
  ...
  if (action === "extend") {
    ...
    auditEvent("license_extended", hash, { days });
  ...
  auditEvent("license_revoked", hash, {});
