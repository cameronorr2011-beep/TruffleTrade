...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="mx-auto max-w-[760px] px-5 py-20 sm:px-8">
      ...
      <h1 className="font-display mt-4 text-[clamp(2rem,4.6vw,3.2rem)] font-semibold leading-[1.08] text-ink">
        {post.title}
      </h1>
      ...
      <div className="mt-10 space-y-8">
        {post.body.map((section, i) => (
          <section key={i}>
            {section.heading && (
              <h2 className="font-display text-[1.45rem] font-semibold text-truffle-600">{section.heading}</h2>
            )}
      ...
      <div className="mt-14 border-t border-soil-600 pt-8">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {others.map((p) => (
            <Link key={p.slug} href={`/blog/${p.slug}`} className="card block p-5 hover:border-forest/35">
